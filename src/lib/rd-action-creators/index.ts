...
export function defineAction<TActionFields = {}>(type: string | { type: string; subtype: string }, defaults?: Partial<TActionFields>): ActionCreator<TActionFields, TActionFields> {
    return defineActionFactory<TActionFields, TActionFields>(
        type,
        fields => ({ ...(defaults || {} as any), ...(fields as any) })
    );
}

export function defineActionFactory<TParams, TActionFields=TParams>(
    type: string | { type: string; subtype: string },
    factory: (params: TParams) => TActionFields): ActionCreator<TParams, TActionFields> {

    type = typeof type === "string" ? generateActionType(type, "") : generateActionType(type.type, type.subtype);

    const creator: ActionCreator<TParams, TActionFields> = ((params: TParams) => ({
...
export interface ApiCallActionSet<TParams, TResult, TError> {
    run: ActionCreator<TParams, TParams>;
    ok: ActionCreator<{ params: TParams; result: TResult }>;
    error: ActionCreator<TError>;
}

export function defineApiCallAction<TParams, TResult, TError = any>(type: string): ApiCallActionSet<TParams, TResult, TError> {
    return {
        run: defineAction<TParams>({ type, subtype: "run" }),
        ok: defineAction<{ params: TParams; result: TResult; }>({ type, subtype: "ok" }),
        error: defineAction<TError>({ type, subtype: "error" })
    };
}
